...
static int op_change_directory(struct BrowserPrivateData *priv, int op)
{
  ...
  {
    int rc = buf_get_field(_("Chdir to: "), buf, MUTT_COMP_FILE, false, NULL, NULL, NULL, NULL);
...
static int op_enter_mask(struct BrowserPrivateData *priv, int op)
{
  ...
  if (buf_get_field(_("File Mask: "), buf, MUTT_COMP_NO_FLAGS, false, NULL, NULL, NULL, NULL) != 0)
  {
    buf_pool_release(&buf);
    return FR_NO_ACTION;
  }
...
static int op_subscribe_pattern(struct BrowserPrivateData *priv, int op)
{
  ...
  if ((buf_get_field(tmp2, buf, MUTT_COMP_PATTERN, false, NULL, NULL, NULL, NULL) != 0) ||
      buf_is_empty(buf))
  {
    buf_pool_release(&buf);
    return FR_NO_ACTION;
