...
static int op_attachment_edit_content_id(struct ComposeSharedData *shared, int op)
{
  ...
  if (buf_get_field("Content-ID: ", buf, MUTT_COMP_NO_FLAGS, false, NULL, NULL, NULL, NULL) == 0)
  {
...
static int op_attachment_edit_description(struct ComposeSharedData *shared, int op)
{
  ...
  if (buf_get_field("Description: ", buf, MUTT_COMP_NO_FLAGS, false, NULL, NULL, NULL, NULL) == 0)
  {
...
static int op_attachment_edit_language(struct ComposeSharedData *shared, int op)
{
  ...
  if (buf_get_field("Content-Language: ", buf, MUTT_COMP_NO_FLAGS, false, NULL, NULL,
                    NULL, NULL) == 0)
  {
...
static int op_attachment_new_mime(struct ComposeSharedData *shared, int op)
{
  ...
  if ((buf_get_field(_("New file: "), fname, MUTT_COMP_FILE, false, NULL, NULL, NULL, NULL) != 0) ||
      buf_is_empty(fname))
  {
    goto done;
  }
  ...
  type = buf_pool_get();
  if ((buf_get_field("Content-Type: ", type, MUTT_COMP_NO_FLAGS, false, NULL, NULL, NULL, NULL) != 0) ||
      buf_is_empty(type))
  {
    goto done;
  }
...
static int op_attachment_rename_attachment(struct ComposeSharedData *shared, int op)
{
  ...
  int rc = buf_get_field(_("Send attachment with name: "), fname,
                         MUTT_COMP_FILE, false, NULL, NULL, NULL, NULL);
  if (rc == 0)
  {
...
static int op_compose_rename_file(struct ComposeSharedData *shared, int op)
{
  ...
  buf_pretty_mailbox(fname);
  if ((buf_get_field(_("Rename to: "), fname, MUTT_COMP_FILE, false, NULL, NULL, NULL, NULL) == 0) &&
      !buf_is_empty(fname))
  {
    struct stat st = { 0 };
