...
void dot_type_nntp_status(FILE *fp, const char *name, int status)
{
  char *str = NULL;
  switch (status)
  {
...
void dot_account_nntp(FILE *fp, struct NntpAccountData *adata, struct ListHead *links)
{
  ...
  dot_type_file(fp, "newsrc_fp", adata->newsrc_fp);
  ...
  dot_type_nntp_status(fp, "status", adata->status);
