...
void dot_account_config(FILE *fp, struct Account *a, struct ListHead *links)
{
  struct Buffer *value = mutt_buffer_alloc(256);
  dot_object_header(fp, a->name, "Account Config", "#ffff80");
  for (size_t i = 0; i < a->num_vars; i++)
  {
    mutt_buffer_reset(value);
    account_get_value(a, i, value);
    dot_type_string(fp, a->var_names[i], value->data);
  }
  dot_object_footer(fp);
...
void dot_account(FILE *fp, struct Account *a, struct ListHead *links)
{
  ...
  if (a->name)
  {
    dot_account_config(fp, a, links);
    dot_add_link(links, a, a->name, "Account Config", false);
  }
#endif
