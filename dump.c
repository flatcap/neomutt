...
void dump_address(const char *label, struct AddressList *al, int i)
{
  if (!al)
    return;

  struct Address *addr = TAILQ_FIRST(al);
  if (!addr)
    return;
...
void dump_header(struct Email *e, int i)
{
  ...
  {
    printf("%*senv: %p\n", i, "", (void *) e->env);
    dump_envelope(e->env, i + 4);
  }
  if (e->content)
  {
    printf("%*scontent: %p\n", i, "", (void *) e->content);
    dump_body(e->content, i + 4);
  }
}
