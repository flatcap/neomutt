...
enum ImapAuthRes imap_auth_sasl(struct ImapAccountData *adata, const char *method)
{
  ...
  while (rc == SASL_CONTINUE || olen > 0)
  {
    ...
    while (irc == IMAP_RESP_CONTINUE);

    if ((irc == IMAP_RESP_BAD) || (irc == IMAP_RESP_NO))
      goto bail;

    if (irc == IMAP_RESP_RESPOND)
    {
  ...
  while (irc != IMAP_RESP_OK)
  {
    irc = imap_cmd_step(adata);
    if (irc != IMAP_RESP_CONTINUE)
      break;
  }
