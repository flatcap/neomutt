...
static int find_command(const char *cmd, int cmd_len)
{
  ...
  for (int i = 0; Commands[i].name; i++)
  {
    ...
    if (mutt_str_strncmp(cmd, Commands[i].name, cmp_len) == 0)
      return i;
  }
...
int mutt_parse_rc_line(/* const */ char *line, struct Buffer *token, struct Buffer *err)
{
  ...
  *err->data = 0;
  ...
  while (*expn.dptr)
  {
    ...
    int idx = find_command(token->data, mutt_str_strlen(token->data));
    if (idx < 0)
    {
...
int mutt_command_complete(char *buffer, size_t len, int pos, int numtabs)
{
  ...
  else if ((mutt_str_strncmp(cmd, "set", 3) == 0) ||
           (mutt_str_strncmp(cmd, "unset", 5) == 0) ||
           (mutt_str_strncmp(cmd, "reset", 5) == 0) ||
           (mutt_str_strncmp(cmd, "toggle", 6) == 0))
  { /* complete variables */
    static const char *const prefixes[] = { "no", "inv", "?", "&", 0 };

    pt++;
    /* loop through all the possible prefixes (no, inv, ...) */
    if (mutt_str_strncmp(cmd, "set", 3) == 0)
    {
  ...
  else if (mutt_str_strncmp(cmd, "exec", 4) == 0)
  {
