...
static void print_enriched_string(int index, int attr, unsigned char *s, int do_color)
{
              ...
              else
              {
                addch(ACS_BTEE);
                    ...
                    static void menu_pad_string(struct Menu * menu, char *s, size_t n)
                    {
                      ...
                      mutt_simple_format(s, n, cols, cols, FMT_LEFT, ' ',
                                         scratch, mutt_strlen(scratch), 1);
                      s[n - 1] = '\0';
                      FREE(&scratch);
                    }
   ...
   * position the cursor for drawing. */
                      ...
                      mutt_window_move(menu->indexwin,
                                       menu->oldcurrent + menu->offset - menu->top, 0);
                      ATTRSET(old_color);
                      ...
                      if (option(OPT_ARROW_CURSOR))
                      {
                        ...
                        if (menu->redraw & REDRAW_MOTION_RESYNCH)
                        {
                          ...
                          menu_pad_string(menu, buf, sizeof(buf));
                          mutt_window_move(menu->indexwin,
                                           menu->oldcurrent + menu->offset - menu->top, 3);
                          print_enriched_string(menu->oldcurrent, old_color,
                                                (unsigned char *) buf, 1);
                        }

                        /* now draw it in the new location */
                        SETCOLOR(MT_COLOR_INDICATOR);
                        mutt_window_mvaddstr(menu->indexwin,
                                             menu->current + menu->offset - menu->top,
                                             0, "->");
                      }
                      else
                      {
                        ...
                        SETCOLOR(MT_COLOR_INDICATOR);
                        mutt_window_move(menu->indexwin,
                                         menu->current + menu->offset - menu->top, 0);
                        print_enriched_string(menu->current, cur_color,
                                              (unsigned char *) buf, 0);
                      }
                      menu->redraw &= REDRAW_STATUS;
                      NORMAL_COLOR;
                    ...
                    void menu_redraw_current(struct Menu * menu)
                    {
                      ...
                      if (option(OPT_ARROW_CURSOR))
                      {
                        ...
                        addch(' ');
                        menu_pad_string(menu, buf, sizeof(buf));
                        print_enriched_string(menu->current, attr,
                                              (unsigned char *) buf, 1);
                      }
                      else
                      {
                    ...
                    void menu_check_recenter(struct Menu * menu)
                    {
                      ...
                      else
                      {
                        ...
                        else
                        {
                          if (menu->current < menu->top + c)
                          {
                            menu->top -= (menu->pagelen - c) *
                                             ((menu->top + menu->pagelen - 1 - menu->current) /
                                              (menu->pagelen - c)) -
                                         c;
                          }
                          else if ((menu->current >= menu->top + menu->pagelen - c))
                          {
                            menu->top +=
                                (menu->pagelen - c) * ((menu->current - menu->top) /
                                                       (menu->pagelen - c)) -
                                c;
                          }
                        }
                    ...
                    static void menu_jump(struct Menu * menu)
                    {
                      ...
                      if (menu->max)
                      {
                        ...
                        buf[0] = '\0';
                        if (mutt_get_field(_("Jump to: "), buf, sizeof(buf), 0) == 0 &&
                            buf[0])
                        {
                    ...
                    static void menu_length_jump(struct Menu * menu, int jumplen)
                    {
                      ...
                      if (menu->max)
                      {
                            ...
                            (tmp = (neg ? 0 : (menu->max /* -1 */) - (menu->pagelen /* -1 */))))
                        {
                          ...
                          /* jumped too long? */
                          if ((neg || !option(OPT_MENU_MOVE_OFF)) &&
                          ...
                          if ((DIRECTION *
                               (tmp = (menu->current -
                                       (menu->top + (neg ? (menu->pagelen - 1) - c : c))))) < 0)
                          {
                            menu->current -= tmp;
                          }
...
#undef DIRECTION
                    ...
                    void mutt_pop_current_menu(struct Menu * menu)
                    {
                      ...
                      {
                        mutt_debug(1, "mutt_pop_current_menu() called with "
                                      "inactive menu\n");
                        return;
                      }
...
#define MUTT_SEARCH_DOWN 2
     ...
     */
                        if (menu->tagprefix && i != OP_TAG_PREFIX &&
                            i != OP_TAG_PREFIX_COND && i != -2)
                        {
                        ...
                        {
                          mutt_window_move(menu->indexwin,
                                           menu->current - menu->top + menu->offset, 2);
                        }
                        else if (option(OPT_BRAILLE_FRIENDLY))
                        {
                          mutt_window_move(menu->indexwin,
                                           menu->current - menu->top + menu->offset, 0);
                        }
                        else
                        {
                          mutt_window_move(menu->indexwin,
                        ...
                        switch (i)
                        {
                          ...
                          case OP_JUMP:
                            ...
                            {
                              mutt_error(
                                  _("Jumping is not implemented for dialogs."));
                            }
                            else
                            {
                              menu_jump(menu);
