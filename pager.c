      ...
      else if ((check == MUTT_NEW_MAIL) || (check == MUTT_REOPENED) || (check == MUTT_FLAGS))
      {
        ...
        if ((check == MUTT_NEW_MAIL) || (check == MUTT_REOPENED))
        {
          if (rd.index && Context)
          {
            ...
            index_hint = Context->mailbox->hdrs[Context->mailbox->v2r[rd.index->current]]->index;
            ...
            if (extra->hdr != Context->mailbox->hdrs[Context->mailbox->v2r[rd.index->current]])
            {
              extra->hdr = Context->mailbox->hdrs[Context->mailbox->v2r[rd.index->current]];
              break;
            }
          }
